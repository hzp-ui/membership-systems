/**
 * @file test_all_rpc_v2.mjs - 完整 RPC 测试（修正版）
 * @description 使用正确的函数签名测试所有 RPC
 * @usage node test_all_rpc_v2.mjs
 */
...
async function login() {
  ...
  if (!data.session) throw new Error('登录失败: 无会话数据');
  
  console.log(`✅ 登录成功! Token: ${data.session.access_token.substring(0, 30)}...`);
...
async function runTests(admin) {
  ...
  log('rpc_get_current_admin_info', r.error ? 'fail' : 'pass', r.error?.message || `name=${r.data?.name}`);

  // ---- 2. 门店 CRUD ----
  console.log('\n--- 2. 门店 CRUD ---');

  r = await rpc('rpc_get_stores', { p_store_id: null });
  ...
  r = await rpc('rpc_get_admins', { p_store_id: null });
  ...
  r = await rpc('rpc_get_members', { p_store_id: null });
  ...
  r = await rpc('rpc_update_member', {
    p_member_id: (await rpc('rpc_get_members', { p_store_id: null })).data?.[0]?.id,
    p_name: '测试会员更新',
    p_phone: testPhone,
  });
  ...
  r = await rpc('rpc_get_barbers', { p_store_id: null });
  ...
  r = await rpc('rpc_get_services', { p_store_id: null });
  ...
  r = await rpc('rpc_get_service_types', { p_store_id: null });
  ...
  r = await rpc('rpc_get_packages', { p_store_id: null });
  ...
  r = await rpc('rpc_get_recharge_records', { p_store_id: null });
  log('rpc_get_recharge_records', r.error ? 'fail' : 'pass', r.error?.message || `${Array.isArray(r.data) ? r.data.length : 0} 条`);

  const members = (await rpc('rpc_get_members', { p_store_id: null })).data;
  ...
  r = await rpc('rpc_get_consume_records', { p_store_id: null });
  log('rpc_get_consume_records', r.error ? 'fail' : 'pass', r.error?.message || `${Array.isArray(r.data) ? r.data.length : 0} 条`);

  if (firstMemberId) {
    const services = (await rpc('rpc_get_services', { p_store_id: null })).data;
    const firstServiceId = services?.[0]?.id;
    const barbers = (await rpc('rpc_get_barbers', { p_store_id: null })).data;
  ...
  r = await rpc('rpc_get_appointments', { p_store_id: null });
...
async function main() {
  try {
    ...
    const adminInfo = await sb.rpc('rpc_get_current_admin_info');
    await runTests(authData.user);
  } catch (err) {
