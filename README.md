...
| **Supabase** | - | BaaS 平台（托管 PostgreSQL + API 网关） |
| **pgcrypto** | - | PostgreSQL 加密扩展（bcrypt 哈希） |

### 数据库特性

| 特性 | 用途 |
|------|------|
| **Stored Procedures (RPC)** | 业务逻辑封装，27 个自定义函数 |
| **SECURITY DEFINER** | 权限提升，绕过 RLS |
| **Row Level Security (RLS)** | 数据隔离（已配置但未启用） |
| **Triggers** | 自动更新时间戳（updated_at） |
| **Indexes** | 查询性能优化 |
| **Views** | 复杂查询封装 |
...
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐     │
│  │ rpc_login   │  │ rpc_members │  │ rpc_stores  │ ... │
│  └─────────────┘  └─────────────┘  └─────────────┘     │
│                        │                                │
│                        ▼                                │
│  ┌──────────────────────────────────────────────────┐  │
│  │              数据表 (stores, admins, members...) │  │
│  └──────────────────────────────────────────────────┘  │
│                        │                                │
│                        ▼                                │
│  ┌──────────────────────────────────────────────────┐  │
│  │              安全层 (bcrypt, audit_logs)          │  │
│  └──────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────┘
```
...
| 依赖 | 用途 |
|------|------|
| `pgcrypto` | bcrypt 密码哈希、加密函数 |
| `uuid-ossp` | UUID 生成 |
| `pg_stat_statements` | 查询性能监控（可选） |

### 开发工具

| 工具 | 用途 |
|------|------|
| Supabase CLI | 本地开发、迁移管理 |
| Supabase SQL Editor | 在线 SQL 执行 |
| psql | 命令行客户端 |
| Node.js | 测试脚本运行 |
